...
def main(cfg):
    ...
    if cfg.exclude:
        train_dataset, test_ref_loader_in, test_loader_in, test_ref_loader_ex, test_loader_ex = dataloader_gen.gen_dataloader(cfg)
    else:
    ...
    if cfg.test_first:
        ...
        if cfg.exclude:
            ...
            srcc, pcc, mae = evaluation(cfg, net, test_ref_loader_in, test_loader_in)
            sys.stdout.write(f'IN - [SRCC: {srcc:.4f}] [PCC: {pcc:.4f}] [MAE: {mae:.4f}] \n')
            write_log(cfg.log_file, '\n[*] External Test')
            srcc, pcc, mae = evaluation(cfg, net, test_ref_loader_ex, test_loader_ex)
